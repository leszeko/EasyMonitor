...
echo
if [[ "no" == $(r_ask "$GREEN install additinal tools, Please answer or Enter for skip$Red") ]]; then echo "$n$Orange Skipped install additinal tools$Reset"
else echo "$Reset"
	...
	echo "$n$Green # /usr/bin/zypper : sudo /usr/bin/zypper install -R ...$Reset$n"
	...
	echo "$n$Green # /sbin/apt-get : su -c apt-get install ...$Reset$n"
	...
	echo "$n$Green # /usr/bin/apt-get : su -c apt-get install ...$Reset$n"
	...
	echo "$n$Green # /usr/sbin/urpmi : su -c urpmi ...$Reset$n"
	...
	echo "$n$Green # /usr/bin/pacman : sudo pacman -Su --needed ...$Reset$n"
	...
	echo "$n$Green # /usr/bin/yum : sudo yum install ...$Reset$n"
	...
	echo "$n$Green # /usr/bin/equo : sudo equo install ...$Reset$n"
	...
	echo "$n$Green # /usr/bin/usm : /usr/bin/usm -g ...$Reset$n"
...
comment=cat<<-EOF
	...
	echo "$n$Green # /usr/bin/eopkg upgrade : sudo eopkg upgrade... $Reset$n"
	sudo eopkg upgrade
...
EOF
	...
	echo "$n$Green # /usr/sbin/netpkg : su -c /usr/sbin/netpkg...$Reset$n"
	...
	echo "$n$Green # /usr/sbin/slackpkg : su -c slackpkg install ...$Reset$n"
	...
	echo "$n$Green # /usr/sbin/slapt-get : su -c slapt-get --install ...$Reset$n"
	...
	echo "$n$Green # /usr/bin/xbps-install : su -c /usr/bin/xbps-install...$Reset$n"
	...
	echo "$n$Green # Done #$Reset$n"
	
	which superkaramba
	[ $? = 0 ] || {
	echo "$n$Red Can't find superkaramba package. Try install it manualy ! $Reset"
	echo "$n$Green Superkaramba is considered to be \"legacy\" and some new distros don't provide it in standard repos"
	echo " So you have to look for it on your own.$n"
	...
	echo "$n For example: superkaramba-15.08.3-1.1.x86_64.rpm package works on Sabayon"
...
if [[ "no" == $(r_ask "$GREEN configure Lm sensors, Please answer or Enter for skip$Red") ]]; then echo "$n$Orange Skipped configure Lm sensors$Reset"
else echo "$Reset"
