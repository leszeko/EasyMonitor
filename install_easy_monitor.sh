...
if [[ $TERM != *xterm* ]]; then SMOOTHBLUE=$BLUE;Orange=$Yellow;ORANGE=$YELLOW;Cream=$Magenta;CREAM=$MAGENTA;else
...
Nline=""$'\n'"" Blink=""$'\033[5m'"" Reset=""$'\033[0;0m'"" Erasel=""$'\033[K'"" Back=""$'\b'"" Creturn=""$'\033[D'"" Ctabh=""$'\033[\t'"" Ctabv=""$'\033[\v'"" Savecp=""$'\033[s'"" Restorecp=""$'\033[u'""
Moveup=""$'\033[1A'"" Movedn=""$'\033[1B'"" Movefd=""$'\033[1C'"" Movebk=""$'\033[1D'""
Linesup () { echo -n ""$'\033['$1'A'"" ;}; Linesdn () { echo -n ""$'\033['$1'B'"" ;}; Charsfd () { echo -n ""$'\033['$1'C'"" ;}; Charsbk () { echo -n ""$'\033['$1'D'"" ;}
#################
...
echo
	...
	while true; do
	read -p "$Orange Please answer, $Green{Y}es  $Red{N}o$SmoothBlue or {u}npack $RED$Blink?:$Reset$Yellow $Movebk" -n 1 -r REPLY
	...
	then
		echo
		echo "You have to install from directory named $RED$Blink "EasyMonitor_install" copy files... $Reset"
		echo
	...
	echo "$n$GREEN Your user login name is $Magenta $you_user_login $GREEN And belong to group:$Cyan $(groups $you_user_login)$Reset"
	echo "$n$RED Give root paswoord if you need:$GREEN add $Magenta$you_user_login$GREEN to$Cyan sudo, users, wheel$GREEN group$RED or Enter for skip.$Reset"
		...
		then :
			if [ $(id -u) = 0 ]
			then
				echo "$n$Orange You are root now skipp...$Reset$n"
			else
				su -c "/bin/bash add_user_to_groups.sh $you_user_login"
				if [ $? != 0 ];then echo "$n$Orange Skipped add user to sudo users group$Reset$n";fi
			fi
			###################
			...
			echo "$MAGENTA Now you are going to modify sudoers file to full work all features for Easy Monitor$n$RED Give root paswoord if you need or Enter for skip.$n$Reset"
			su -c "/bin/bash "../EasyMonitor_install/modify_sudoers.sh""
			if [ $? != 0 ];then echo "$n$Orange Skipped modyfy sudoers file$Reset$n";fi
			echo
		fi
	##################
	# Check enviroment
	...
	echo "$n$GREEN"You are going to copy EasyMonitor to "$Themes_Prefix$Easy_Monitor_Work_Dir" directory to full work all features"$Reset$n"
		...
		if [ -d "$Themes_Prefix$Easy_Monitor_Work_Dir" ]; then
		  ...
		  if [ $? = 0 ]; then
			...
			echo "$RED$Blink Error: cd $Themes_Prefix exit 1$Reset"; exit 1
		  fi
		  cd  "$old_pwd"
		fi
		###################
		# Copy Easy Monitor to SuperKaramba themes directory
		mkdir -p "$Themes_Prefix$Easy_Monitor_Work_Dir" || { echo "$RED$Blink Error: mkdir -p $Themes_Prefix$Easy_Monitor_Work_Dir exit 1$Reset"; exit 1 ;}
...
echo "$n$GREEN$Blink The end...$Reset$n"
sleep 10
exit