...
then
case $1 in
"") $P_SCAN -l $X -t $Y -x $_X -y $_Y --resolution $DPI --format pnm >$TMPDIR/EasyMonito-$USER/$NAME$POSTFIX ;;
"color") $P_SCAN --mode color -l $X -t $Y -x $_X -y $_Y --resolution $DPI --format pnm >$TMPDIR/EasyMonito-$USER/$NAME$POSTFIX ;;
"gray") $P_SCAN --mode gray -l $X -t $Y -x $_X -y $_Y --resolution $DPI --format pnm >$TMPDIR/EasyMonito-$USER/$NAME$POSTFIX ;;
"lineart") $P_SCAN --mode lineart -l $X -t $Y -x $_X -y $_Y --resolution $DPI --format pnm >$TMPDIR/EasyMonito-$USER/$NAME$POSTFIX ;;
esac
...
then $P_CONVERT -format $FORM -resize $SIZE -quality $QUALITY $TMPDIR/EasyMonito-$USER/$NAME$POSTFIX

else kdialog --title "Error" --msgbox "You have to check scanner connection and sane-utils configuration"
if [ -f $TMPDIR/EasyMonito-$USER/$NAME$POSTFIX ] ; then rm $TMPDIR/EasyMonito-$USER/$NAME$POSTFIX ; fi
if [ -f $TMPDIR/EasyMonito-$USER/$NAME$POSTFIX.$FORM ] ; then rm $TMPDIR/EasyMonito-$USER/$NAME$POSTFIX.$FORM ;fi
exit 0
fi
...
if [ -f $TMPDIR/EasyMonito-$USER/$NAME$POSTFIX.$FORM ]
then
Counter=0
...
cp $TMPDIR/EasyMonito-$USER/$NAME$POSTFIX.$FORM $DEST/$NAME$POSTFIX$Counter.$FORM
