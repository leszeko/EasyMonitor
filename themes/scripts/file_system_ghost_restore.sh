...
Mount_source_restore ()
{
	...
	do	
		...
		then :
			Mount_Point_Source_Restrore=$TMPDIR/EasyMonitor-$USER/destination_backup
			echo "$Nline$LGreen # Mount source restore:$Reset"
			echo "$Green mount $Source_Restore $TMPDIR/EasyMonitor-$USER/destination_backup"
			mount $Source_Restore $TMPDIR/EasyMonitor-$USER/destination_backup
			if [ $? = 0 ]
			then :
				sleep 0.2
...
Mount_destination_restore ()
{
	...
	echo "$Red mount $Destination_Restore $TMPDIR/EasyMonitor-$USER/source_backup$Reset"
	mount $Destination_Restore $TMPDIR/EasyMonitor-$USER/source_backup
	if [ $? = 0 ]
	then :
...
Restore_backup ()
{
	...
	then :
		...
		then :	
			...
			then :
				Start_time=`date +%s`
				( cd $Mount_Point_Source_Restrore/$Destination_Backup_Dir/$Source_Restore_File && rsync --stats --info=progress2 --info=name0 -aAX  * $TMPDIR/EasyMonitor-$USER/source_backup/ 2>"$Errors")
				Elapsed_time=$((`date +%s` - $Start_time))
			else :
