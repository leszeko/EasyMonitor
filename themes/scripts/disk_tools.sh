...
Info_data ()
{
 ...
 Purpose=" # Colect basic disk tools$Nline"
Purpose+=" #"
...
Restore_backup ()
{
	...
	then :
		...
		then :	
			...
			then :
				Start_time=`date +%s`
				( cd $Mount_Point_Device/$Destination_Backup_Dir/$Source_Restore_File && rsync --stats --info=progress2 --info=name0 -aAX  * $TMPDIR/EasyMonito-$USER/source_backup/ 2>"$Errors")
				Elapsed_time=$((`date +%s` - $Start_time))
			else :
