...
comment[X]=$(cat<<-EOF
Sabayon
Last Update: 2017-02-28 11:10 UTC
...
[Sabayon]
    ...
    Desktop: Fluxbox, GNOME, KDE, MATE, Xfce
    Category: Beginners, Desktop, Live Medium, Raspberry Pi
    Release Model: Rolling
    Status: Active
    Popularity: 53 (228 hits per day) 
...
Popularity (hits per day): 12 months: 44 (253), 6 months: 53 (228), 3 months: 58 (217), 4 weeks: 60 (232), 1 week: 32 (412)

Average visitor rating: 5/10 from 1 review(s).


Sabayon Summary
Distribution 	Sabayon (formerly Sabayon Linux, before that RR4 Linux and RR64 Linux)
...
Screenshots 	http://www.sabayon.org/screenshots • LinuxQuestions.org • DistroWatch Gallery
Screencasts 	LinuxQuestions.org
Download Mirrors 	http://www.sabayon.org/download • LinuxTracker.org
Bug Tracker 	http://bugs.sabayon.org/
Related Websites 	Sabayon Brazil • Wikipedia
Reviews 	15: DistroWatch • Ordinatechnic • DistroWatch
14: Ordinatechnic
13: Blogspot
11: Dedoimedo • DistroWatch • Linux Za Sve (Croatian) • Wordpress • Blogspot • Blogspot • Blogspot
10: Linux Za Sve (Croatian) • Blogspot
9: DistroWatch • Linux User
8: DistroWatch • Wordpress • Linux User • Dedoimedo • LinuxBSDos
7: LinuxBSDos • DistroWatch • Linux User • LinuxBSDos
6: My Linux Rig • Blogspot • LinuxBSDos • LinuxBSDos • Techgage
5.x: LinuxBSDos • DistroWatch • Dedoimedo • LinuxBSDos • LinuxBSDos • Linux Journal • LinuxBSDos • ABC Linuxu (Czech) • DistroWatch
4.x: Dedoimedo • Adventures in Open Source • LWN
3.x: It's A Binary World • Dedoimedo • Viva o Linux (Portuguese) • Linuxoid (Russian) • Blogspot • Linux.com • Techgage • Tuxmachines • Techgage
2.x: Tuxmachines
Where To Buy 	OSDisc.com (sponsored link)
EOF
)
#
home_page[X]="http://www.sabayon.org/"
#
distro[X]="Sabayon_Linux_"							# fitst distro[X] = null end array of distros/packages..
version[X]="latest_"
arch[X]="amd64_"
desktop[X]="KDE"
file_type[X]=".iso"
...
# gpg_file[X]="${gpg[X]}"
install_folder[X]="/ISO/${distro[X]}${version[X]}iso"	# folder for download iso file from mirros
#
: ${mirrors[X]="
 http://ftp.portlane.com/pub/os/linux/sabayon/iso/monthly/
 http://mirror2.mirror.garr.it/mirrors/sabayonlinux/iso/monthly/
"}
#
...
if [ $? = 0 ]
then :
	LATEST_IS=$(wget -O - ftp://ftp.portlane.com/pub/os/linux/sabayon/iso/monthly/LATEST_IS)
	version[X]="$LATEST_IS"
	echo "$Green Latest is - $SmoothBlue$LATEST_IS$Reset"
	file_name[X]="${distro[X]}${version[X]}_${arch[X]}${desktop[X]}${file_type[X]}"	# the file to downloda - comment name to disable
...
kernel_x="/boot/sabayon"
iso_command_x="isoboot="
append_x="root=/dev/ram0 init=/linuxrc cdroot rd.vconsole.keymap=${bootkeymap} vconsole.keymap=${bootkeymap} locale=${bootlang} ${modeset_opt} ${acpi_opt} looptype=squashfs loop=/livecd.squashfs overlayfs splash quiet"
initrd_x="/boot/sabayon.igz"
add_to_grub_menu
add_to_xxx_menu
add_menu_label_x="Live (Troubleshooting)"
append_x="root=/dev/ram0 init=/linuxrc cdroot rd.vconsole.keymap=${bootkeymap} vconsole.keymap=${bootkeymap} locale=${bootlang} ${modeset_opt} ${acpi_opt} looptype=squashfs loop=/livecd.squashfs overlayfs systemd.unit=multi-user.target installer-text splash quiet"
add_to_grub_menu
add_to_xxx_menu
}
...
change passwor for root: sudo su && passwd$Reset
"
#
#!FIXME ### --- New Item ------ ###
X=2
