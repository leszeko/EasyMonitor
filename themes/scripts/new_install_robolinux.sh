...
[Robolinux]
    ...
    Desktop: Cinnamon, GNOME, KDE, LXDE, Xfce
    Category: Beginners, Desktop, Live Medium
    Release Model: Fixed
    Status: Active
    Popularity: 74 (174 hits per day) 

Distribution 	Robolinux
Home Page 	http://www.robolinux.org/
Mailing Lists 	--
User Forums 	http://sourceforge.net/p/robolinux/discussion/
Alternative User Forums 	LinuxQuestions.org
Documentation 	http://www.robolinux.org/install/
Screenshots 	LinuxQuestions.org • DistroWatch Gallery
Screencasts 	LinuxQuestions.org
Download Mirrors 	http://robolinux.org/free/ • SourceForge • LinuxTracker.org
EOF
)
#
home_page[X]="http://www.robolinux.org/"
#
distro[X]="robolinux"								# fitst distro[X] = null end array of distros/packages..
version[X]="32-mate-v8.7.1"
desktop[X]=""
arch[X]=""
file_type[X]=".iso"
...
sum[X]="010fc336138c4366e602c60b29f9eb0c" 				# file with sum to download or sum
...
#
: ${mirrors[X]="
 https://heanet.dl.sourceforge.net/project/robolinux/
 "}
#
...
add_to_grub_menu
add_to_xxx_menu
add_menu_label_x="Install gui"
kernel_x="/install/gtk/vmlinuz"
append_x="video=vesa:ywrap,mtrr vga=788"
...
#
distro[X]="robolinux"								# fitst distro[X] = null end array of distros/packages..
version[X]="32-cinnamon-v8.7.1"
desktop[X]=""
arch[X]=""
file_type[X]=".iso"
file_name[X]="${distro[X]}${version[X]}${desktop[X]}${arch[X]}${file_type[X]}"	# the file to downloda - comment name to disable - end of playing at split name in sections :)
sum[X]="030901c466386a38eabce8748b1f968e" 				# file with sum to download or sum
# sum_file[X]="${sum[X]}"							# comment file name to disable if sum is set
# gpg[X]="${file_name[X]}.sig"
# gpg_file[X]="${gpg[X]}"
install_folder[X]="/ISO/${distro[X]}${version[X]}${desktop[X]}${arch[X]}_iso"	# folder for download iso file from mirros
