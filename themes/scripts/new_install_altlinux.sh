...
comment[X]=$(cat<<-EOF
ALT Linux
Last Update: 2017-02-23 05:22 UTC

[ALT Linux]

    OS Type: Linux
    Based on: Independent (forked from Mandrake)
    Origin: Russia
    Architecture: armv7, i586, x86_64
    Desktop: AfterStep, Blackbox, Enlightenment, FVWM, GNOME, IceWM, KDE, LXDE, MATE, Openbox, Razor-qt, WMaker, Xfce
    Category: Desktop, Education, Live Medium, Server
    Release Model: Fixed
    Status: Active
    Popularity: 113 (102 hits per day) 
...
Popularity (hits per day): 12 months: 123 (84), 6 months: 113 (102), 3 months: 98 (139), 4 weeks: 137 (72), 1 week: 124 (74)
...
Home Page 	http://www.altlinux.com/
Mailing Lists 	http://lists.altlinux.org/
User Forums 	http://forum.altlinux.org/
Alternative User Forums 	LinuxQuestions.org
Documentation 	http://www.altlinux.org/Main_Page (Russian)
http://heap.altlinux.ru/ (Russian)
Screenshots 	LinuxQuestions.org • DistroWatch Gallery
Screencasts 	LinuxQuestions.org
Download Mirrors 	http://www.altlinux.com/en/go/download •
Bug Tracker 	https://bugzilla.altlinux.org/
Related Websites 	AltLinux.ru • Sisyphus Repository • Wikipedia
Reviews 	5.x: LinuxBSDos • LinuxBSDos
IXBT (Russian) • Computerra (Russian)
Where To Buy 	OSDisc.com (sponsored link)

http://ftp.altlinux.org/pub/distributions/ALTLinux/p8/images/kworkstation/
alt-kworkstation-8.1-live-i586.iso 
alt-kworkstation-8-live-i586.iso
MD5SUM 
ndex of /pub/distributions/ALTLinux/
http://wiki.mate-desktop.org/download
EOF
)
#
home_page[X]="http://www.altlinux.com/"
#
distro[X]="alt"								# fitst distro[X] = null end array of distros/packages..
version[X]="-live-1.8.2"
desktop[X]=""
arch[X]="-i586"
file_type[X]=".iso"
...
kernel_x="/syslinux/alt0/vmlinuz"
iso_command_x="automatic=method:disk,uuid:$uuid,directory:"
append_x="fastboot live ramdisk_size=2043709 stagename=live showopts lowmem vga=0x314 quiet splash lang=pl_PL"
initrd_x="/syslinux/alt0/full.cz"
add_to_grub_menu
add_to_xxx_menu
add_menu_label_x="Live session"
append_x="fastboot live ramdisk_size=2043709 stagename=live showopts live_rw vga=0x314 quiet splash lang=pl_PL"
add_to_grub_menu
add_to_xxx_menu
}
...
#
distro[X]="alt"								# fitst distro[X] = null end array of distros/packages..
version[X]="-kworkstation-8.1-live"
desktop[X]=""
arch[X]="-i586"
file_type[X]=".iso"
file_name[X]="${distro[X]}${version[X]}${desktop[X]}${arch[X]}${file_type[X]}"	# the file to downloda - comment name to disable - end of playing at split name in sections :)
