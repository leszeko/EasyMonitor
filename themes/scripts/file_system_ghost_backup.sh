...
Mount_source_backup ()
{
	...
	echo "$Nline$Green Mount$Red source$Green backup: $LRed„$Source_backup”$Green on: $Magenta$TMPDIR/EasyMonito-$USER/Source_backup :$Reset"
	mount -v $Source_backup $TMPDIR/EasyMonito-$USER/Source_backup
	if [ ! $? = 0 ]
	then :
...
Mount_destination_backup ()
{
	...
	then :
		Mount_Point_Destination_backup=$TMPDIR/EasyMonito-$USER/Destination_backup
		echo "$Nline$Green Mount$Yellow destination$Green backup: $Cyan„$Destination_backup”$Green on: $LBlue$TMPDIR/EasyMonito-$USER/Destination_backup :$Reset"
		mount -v $Destination_backup $TMPDIR/EasyMonito-$USER/Destination_backup 2>&1
...
Label_volume ()
{
	...
	then :
		...
		mount $Source_backup $TMPDIR/EasyMonito-$USER/Source_backup
		if [ $? = 0 ]
		then :
			
			Log+="$Green Mount: „$Source_backup” „$TMPDIR/EasyMonito-$USER/Source_backup”$Nline"
		else :
			Log+="$LRed Erorr: mount: „$Source_backup” „$TMPDIR/EasyMonito-$USER/Source_backup”, exit$Nline"
			exit 1
		fi
	fi
...
Ask_for_store_additional_informations ()
{
	...
	then :	
		...
		blkid $Source_backup > $TMPDIR/EasyMonito-$USER/Source_backup/EM_Backup_log.txt
		echo "$Yellow df -h $Source_backup >> EM_Backup_log.txt$Reset"
		df -h $Source_backup >> $TMPDIR/EasyMonito-$USER/Source_backup/EM_Backup_log.txt
		echo "$Yellow fdisk -l >> EM_Backup_log.txt$Reset"
		fdisk -l >> $TMPDIR/EasyMonito-$USER/Source_backup/EM_Backup_log.txt
		echo "$Yellow Date `date` >> EM_Backup_log.txt$Reset"
		echo -n "Date " >> $TMPDIR/EasyMonito-$USER/Source_backup/EM_Backup_log.txt
		date >> $TMPDIR/EasyMonito-$USER/Source_backup/EM_Backup_log.txt
		
		echo "$Nline$Yellow dd if=$Device_Name  of=„$Source_backup”/EM_$Device_UUID-MBR.img bs=446 count=1 conv=notrunc$Reset"
		dd if=$Device_Name  of=$TMPDIR/EasyMonito-$USER/Source_backup/EM_$Device_UUID-MBR.img bs=446 count=1 conv=notrunc
		
		echo "$Yellow dd if=$Device_Name  of=„$Source_backup”/EM_$Device_UUID-DPT.img bs=1 skip=446 count=64 conv=notrunc$Reset"
		dd if=$Device_Name  of=$TMPDIR/EasyMonito-$USER/Source_backup/EM_$Device_UUID-DPT.img bs=1 skip=446 count=64 conv=notrunc
		...
		dd if=$Device_Name of=$TMPDIR/EasyMonito-$USER/Source_backup/EM_$Device_UUID-GAP.img bs=512 skip=1 count="$Device_Boot_Gap_Record" conv=notrunc
	else :	
...
Make_backup ()
{
	...
	Source=$TMPDIR/EasyMonito-$USER/Source_backup
...
/var/tmp/"
	...
	if [ ! $? = 0 ]
	then :
		echo "$LRed Erorr: can't cd: $TMPDIR/EasyMonito-$USER/Source_backup, exit$Reset$Nline"
		Log+="$Red Erorr: cd: „$TMPDIR/EasyMonito-$USER/Source_backup”, exit$Nline"
		exit 1
	fi
